...
// Cache busting headers for dynamic template updates - DEVELOPMENT MODE
...
if (!isset($_NO_COMPRESS)) if (!ob_start('ob_gzhandler')) ob_start();
    ...
    $cacheBust = '?' . time(); // Force cache busting with timestamp
    if (!empty($stdhead['js'])) {
        foreach ($stdhead['js'] as $JS) $js_incl.= "<script type='text/javascript' src='{$INSTALLER09['baseurl']}/scripts/" . $JS . ".js{$cacheBust}'></script>";
    }
    ...
    if (!empty($stdhead['css'])) {
        foreach ($stdhead['css'] as $CSS) $css_incl.= "<link type='text/css' rel='stylesheet' href='{$INSTALLER09['baseurl']}/templates/{$stylez}/css/" . $CSS . ".css{$cacheBust}' />";
    }
 ...
 function stdfoot($stdfoot = false)
{
    ...
    $debug = (SQL_DEBUG && isset($INSTALLER09['allowed_staff']['id']) && is_array($INSTALLER09['allowed_staff']['id']) && in_array($CURUSER['id'], $INSTALLER09['allowed_staff']['id']) ? 1 : 0);
    ...
    $stats = $mc1->stats();
    if (($MemStats = $mc1->get_value('redis_stats')) === false) {
      $MemStats = $stats;
      if ($MemStats['cmd_get'] != 0) {
        $MemStats['Hits'] = number_format(($MemStats['hits'] / $MemStats['cmd_get']) * 100, 3);
      } else {
        $MemStats['Hits'] = 0;
      }
      $mc1->cache_value('redis_stats', $MemStats, 10);
    }
    // load averages - pdq
    if ($debug) {
  ...
  if ($CURUSER) {
        ...
        $htmlfoot.= "
			<div class='panel panel-default'>	
                ...
                " . ($debug && $CURUSER['class'] === UC_MAX ? "<a title='{$lang['gl_stdfoot_sview']}' rel='external' href='/staffpanel.php?tool=system_view'>{$lang['gl_stdfoot_sview']}</a> | " . "<a rel='external' title='OPCache' href='/staffpanel.php?tool=op'>{$lang['gl_stdfoot_opc']}</a> | " . "<a rel='external' title='Redis' href='/staffpanel.php?tool=redis'>Redis</a>" : "") . "";
